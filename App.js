...
<body>
  ...
  <div>
    <button id="json">Send json to RN</button>
  </div>
...
"use strict";
var _reactNativeWebviewMessaging=__webpack_require__(0);var _reactNativeWebviewMessaging2=_interopRequireDefault(_reactNativeWebviewMessaging);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}var helloBtn=document.querySelector('#hello');var jsonBtn=document.querySelector('#json');var eventBtn=document.querySelector('#event');var messagesContainer=document.querySelector('p');helloBtn.addEventListener('click',function(){_reactNativeWebviewMessaging2.default.send('hello');});jsonBtn.addEventListener('click',function(){_reactNativeWebviewMessaging2.default.sendJSON({payload:'hello'});});eventBtn.addEventListener('click',function(){_reactNativeWebviewMessaging2.default.emit('greetingFromWebview',{payload:'hello'});});_reactNativeWebviewMessaging2.default.on('text',function(text){messagesContainer.innerHTML='Received text from RN: '+text;});_reactNativeWebviewMessaging2.default.on('json',function(text){messagesContainer.innerHTML='Received json from RN: '+JSON.stringify(text);});_reactNativeWebviewMessaging2.default.on('greetingFromRN',function(event){messagesContainer.innerHTML='Received "greetingFromRN" event: '+JSON.stringify(event);});
  ...
  render() {
    return (
      <View style={{ flex: 1 }}>
        ...
        <WKWebView source={{ html: webViewHTML }} style={{ flex: 1 }} ref={this._refWebView} />        
      </View>
    );
  }
...
export default TabNavigator({
  ...
  Webview: {
    screen: () => <WKWebView source={{uri: 'https://homeapp.ru/search_new/#/map/'}} style={{ flex: 1 }}/>
  }
}, {
  tabBarPosition: 'top',
